...
public class UserServiceDAOImpl implements UserServiceDAO {
  ...
  public UserModel getUser(String username, String password) throws DataAccessException {
    Query query = getEntityManager().createQuery("select u from users u where u.username='"+username+"' and u.password='"+password+"'", UserModel.class);
    List<UserModel> resultList = query.getResultList();
  ...
  public Boolean isUsernameRegistered(String username) throws DataAccessException {
    Query query = getEntityManager().createQuery("select u from users u where u.username='"+username+"'", UserModel.class);
    if (!query.getResultList().isEmpty()) {
    	return true;
    } else {
    	return false;
  ...
  public Boolean isEmailRegistered(String email) throws DataAccessException {
    Query query = getEntityManager().createQuery("select u from users u where u.email='"+email+"'", UserModel.class);
    if (!query.getResultList().isEmpty()) {
    	return true;
    } else {
    	return false;
