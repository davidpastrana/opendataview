...
import com.opendataview.web.model.SuggestRouteModel;
...
import com.opendataview.web.panels.RatingsPanel;
...
//	  origPropList = propertiesServiceDAO.readPropertiesModel(session.setAttribute("user_name", "admin"));
//	  
//	  
    ...
    for (int i = 0; i < origList.size(); i++) {
      if (!names.contains(origList.get(i).getCsvName()) && origList.get(i).getUser() != null && origList.get(i).getUser().contentEquals(username)) {
...
//            }
            ...
            for (final LocationModel loc : origList) {
            	...
            	String name = loc.getOther();
            	

            	if(name != null) {
              //if (name.intern().toLowerCase().contains(input.toLowerCase().intern())) {
              if (name.toLowerCase().replaceAll("\\s","").replaceAll("##","").matches("(.*)"+input.toLowerCase().replaceAll("\\s","").replaceAll("##","")+"(.*)")) {
//              	log.info("in field: "+input.toLowerCase().intern());
//              	log.info("db field: "+loc.getOther().toLowerCase().intern());
                choices.add(name);//.replaceAll(" ##",","));
...
//		  coordGeoForm.add(new AttributeModifier("style", "visibility:visible;"));
//	  }
//    } else {
//    	coordGeoForm.add(new AttributeModifier("style", "visibility:hidden;"));
//    }
...
//          Object text;
//          new Label("other", obj.getOther().replaceAll("##", "\n"))
		item.add(new MultiLineLabel("other", obj.getOther().replaceAll("##", "\n")));
        } else {
          item.add(new Label("otherTag").setVisible(false));
          item.add(new Label("other", obj.getSchedule()).setVisible(false));
        }
        if (obj.getDateUpdated() != null) {
            item.add(new Label("dateUpdatedTag", "Last update: "));
            item.add(new Label("dateUpdated", obj.getDateUpdated()));
          } else {
            item.add(new Label("dateUpdatedTag").setVisible(false));
            item.add(new Label("dateUpdated", obj.getDateUpdated()).setVisible(false));
          }
        if (obj.getDatePublished() != null) {
            item.add(new Label("datePublishedTag", "Last publishment: "));
            item.add(new Label("datePublished", obj.getDatePublished()));
          } else {
...
//        		  new File(enriched)));
          ...
          String newFormat = path + "processed/structured/" + obj.getCsvName();
          ...
          String sqlFile = path + "processed/sql/" + obj.getCsvName().replace("csv", "sql");
